...
export interface Cloze {
  ...
  reason: string;
  /**
   * Ответ, когда нужная форма не берётся из таблицы лиц: hay — безличная форма
   * haber, и в спряжении её нет (he, has, ha…). Задавать вручную стоит только
   * ради таких форм, обычные подставляются генератором.
   */
  answer?: string;
}
